...
function isNameLookup(member={}) { return member.lookup_mode === 'name' || member._lookupMode === 'name'; }
...
function renderResult(member, target=$('#verificationResult')) {
  if (!member) { target.innerHTML='<div class="empty"><span>×</span><strong>Data tidak ditemukan</strong><small>Periksa nama atau nomor kartu, lalu hubungi pengurus bila perlu.</small></div>'; return; }
  const logo = $('.brand img')?.src || '';
  const readOnly=isNameLookup(member);
  const actionMarkup=readOnly
    ? `<div class="name-lookup-note"><span aria-hidden="true">◉</span><div><strong>Mode lihat saja</strong><small>Hasil pencarian nama tidak dapat disimpan, diunduh, atau dibagikan. Gunakan QR atau kode kartu pribadi untuk membuka fitur tersebut.</small></div></div><div class="verification-actions view-only-actions"><button class="btn btn-outline zoom-member-card" id="zoomMemberCard" type="button">Perbesar kartu <b>⤢</b></button><button class="btn btn-outline" id="toggleCardSide" type="button">Lihat sisi belakang <b>↔</b></button></div>`
    : `<div class="verification-actions"><button class="btn btn-dark" id="printMemberCard" type="button">Download Kartu PDF <b>↓</b></button><button class="btn btn-dark" id="downloadMemberPng" type="button">Simpan Kartu PNG <b>↓</b></button><button class="btn btn-outline zoom-member-card" id="zoomMemberCard" type="button">Perbesar kartu <b>⤢</b></button><button class="btn btn-outline" id="toggleCardSide" type="button">Lihat sisi belakang <b>↔</b></button><button class="btn btn-outline" id="shareMemberCard" type="button">Bagikan kartu <b>↗</b></button></div>`;
  target.innerHTML = `<div class="verification-card-wrap"><div class="verification-card-visual"><div class="id-card-face verification-id-card" id="verifiedFront"><div class="membership-watermark">MEMBERS ONLY</div><div class="id-card-top"><img class="id-card-logo" src="${safe(logo)}" alt="Logo RCS.CBS HOPE" /><span class="id-card-badge status-${statusClass(member.status)}">${safe(member.status || 'ANGGOTA')}</span></div>${competencyBadgesMarkup(member)}<div class="id-card-main">${cardPhotoMarkup(member)}<div><span class="id-card-label">Nama anggota</span><div class="id-card-name">${safe(member.name).toUpperCase()}</div><div class="id-card-reg">${safe(displayRegistrationNumber(member))}</div><div class="id-card-blood">GOL. DARAH <b>(${safe(member.blood_type)})</b></div><div class="id-card-tagline">PENCINTA ALAM · COMMUNITY MEMBER</div></div></div><div class="id-card-qr">${member.status === 'Aktif' && !isNameLookup(member) && (member.public_token || member.id) ? '<canvas id="resultQr"></canvas>' : '<span class="qr-pending">MENUNGGU<br>VERIFIKASI</span>'}</div><div class="id-card-bottom"><span>RCS.CBS HOPE</span><span>BERLAKU SEUMUR HIDUP</span><span>SELAMA KARTU AKTIF</span><span>MEMBER ID</span></div></div><div class="id-card-back verification-card-back hidden" id="verifiedBack"><img class="back-compass-img" src="./reichas-virtual-stamp.png" alt="Stempel Reichas Chelebes" /><img class="back-house-img" src="legacy-house-forest-gray.jpg" alt="Siluet rumah RCS.CBS" /><div class="back-copy"><span class="back-kicker">REICHAS CHELEBES</span><strong>Bangga menjadi bagian Keluarga RCS.CBS.</strong><p>Di mana pun langkah membawa kita, Keluarga Reichas Chelebes tetap terhubung. Kartu ini adalah identitas anggota—jembatan komunikasi, koordinasi, dan kepedulian saat dibutuhkan.</p></div><div class="back-detail"><div class="back-detail-block"><span>PEMILIK LEGACY</span><strong>${safe(member.parent_name).toUpperCase()}</strong></div><div class="back-detail-block"><span>NAMA ANGKATAN</span><strong>${safe(displayCohortName(member.cohort_name))}</strong></div><div class="back-detail-block back-card-number"><strong>${safe(displayRegistrationNumber(member))}</strong></div></div></div></div><div class="verification-card-info"><div><span>Pemilik legacy</span><strong>${safe(member.parent_name).toUpperCase()}</strong></div><div><span>Golongan darah</span><strong>${safe(member.blood_type)}</strong></div><div><span>Nama angkatan</span><strong>${safe(displayCohortName(member.cohort_name))}</strong></div></div>${actionMarkup}</div>${journeyMapMarkup(member)}`;
  bindCardPhotoFallbacks(target);
  if(member.status === 'Aktif' && !isNameLookup(member) && (member.public_token || member.id)) renderQRCode($('#resultQr'), verifyUrl(member.public_token || member.id));
...
async function openPrintCard(member, mode='pdf') {
  if(isNameLookup(member)){toast('Hasil pencarian nama hanya dapat dilihat. Gunakan QR atau kode kartu pribadi untuk mengunduh.');return;}
  ...
  if (isConfigured()) {
    ...
    if (error) {
      if (error.code === 'PGRST202' || /verify_member/i.test(error.message || '')) {
        throw new Error('Layanan verifikasi belum diperbarui. Jalankan hotfix-v102-name-search.sql di Supabase SQL Editor.');
      }
      throw error;
    }
    const member=Array.isArray(data) ? (data[0] || null) : (data || null);
    return member;
  }
  const normalizedKey=key.replace(/\s+/g,'').toLowerCase();
  const items=demoMembers().filter(m=>m.status==='Aktif');
  const credential=items.find(m=>String(m.registration_number||'').replace(/\s+/g,'').toLowerCase()===normalizedKey || trailingVerificationCode(m.registration_number)===trailingCode || m.public_token===key);
  if(credential) return {...credential,lookup_mode:'credential'};
  if(key.length<3) return null;
  const nameMatches=items.filter(m=>String(m.name||'').toLowerCase().includes(key.toLowerCase()));
  return nameMatches.length===1 ? {...nameMatches[0],id:null,public_token:null,lookup_mode:'name'} : null;
}
$('#verifyForm').addEventListener('submit', async (event)=>{event.preventDefault();const alert=$('#verifyAlert');clearAlert(alert);try{const member=await findMember($('#verifyInput').value);if(!member) {renderResult(null);alertBox(alert,'Data tidak ditemukan, nama belum cukup spesifik, atau kartu belum aktif. Coba nama lengkap atau periksa kode kartu.','error');} else {renderResult(member);alertBox(alert,'Data anggota ditemukan.','success');}}catch(e){alertBox(alert,e.message||'Verifikasi gagal.','error')}});
