...
function openCardPreview(member) {
  ...
  body.innerHTML=`<div class="id-card-preview"><div class="id-card-face"><div class="id-card-top"><img class="id-card-logo" src="${safe(logo)}" alt="Logo RCS.CBS HOPE" /><span class="id-card-badge">${safe(member.status || 'ANGGOTA')}</span></div><div class="id-card-main">${cardPhotoMarkup(member)}<div><span class="id-card-label">Nama anggota</span><div class="id-card-name">${safe(member.name).toUpperCase()}</div><div class="id-card-reg">${safe(member.registration_number || 'Nomor belum diterbitkan')}</div><div class="id-card-blood">GOL. DARAH <b>${safe(member.blood_type)}</b></div></div></div><div class="id-card-qr"><canvas id="modalQr"></canvas></div><div class="id-card-bottom"><span>RCS.CBS HOPE · 2026</span><span>MEMBER ID</span></div></div><div class="id-card-back"><img class="back-compass-img" src="${safe(compass)}" alt="Kompas mata angin" /><div class="back-copy"><strong>Bangga menjadi bagian dari keluarga Reichas.</strong><p>Saling mengenal, saling menjaga. Kartu ini adalah identitas anggota untuk kegiatan Organisasi Reichas Chelebes.</p></div><div class="back-detail"><span>${safe(member.parent_name)}</span><span>${safe(member.cohort_year)} · ${safe(member.blood_type)}</span></div></div></div><div class="modal-actions"><button class="btn btn-dark" id="modalPrintCard" type="button">Download Kartu PDF <b>↓</b></button></div>`;
...
function openPrintCard(member) {
  ...
  const name=xmlEscape(member.name).toUpperCase(); const reg=xmlEscape(member.registration_number || 'Nomor belum diterbitkan'); const blood=xmlEscape(member.blood_type || '-'); const parent=xmlEscape(member.parent_name || '-'); const status=xmlEscape(member.status || 'ANGGOTA'); const photoHref=xmlEscape(photo); const logoHref=xmlEscape(logo); const compassHref=xmlEscape(compass); const qrHref=xmlEscape(qrSrc);
  const frontSvg=`<svg xmlns="http://www.w3.org/2000/svg" width="860" height="540" viewBox="0 0 860 540"><defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#112129"/><stop offset=".62" stop-color="#1b3740"/><stop offset="1" stop-color="#496b70"/></linearGradient></defs><rect width="860" height="540" rx="38" fill="url(#bg)"/><circle cx="760" cy="410" r="210" fill="none" stroke="#ffffff" stroke-opacity=".12" stroke-width="2"/><circle cx="760" cy="410" r="165" fill="none" stroke="#ffffff" stroke-opacity=".08" stroke-width="20"/><text x="478" y="430" fill="#ffffff" fill-opacity=".10" font-family="Arial" font-weight="700" font-size="30" letter-spacing="5" transform="rotate(-18 478 430)">MEMBERS ONLY</text><image href="${logoHref}" x="60" y="48" width="90" height="90" preserveAspectRatio="xMidYMid slice"/><rect x="690" y="52" width="115" height="42" rx="21" fill="none" stroke="#ffffff" stroke-opacity=".65"/><text x="747" y="78" fill="#ffffff" font-family="Arial" font-size="16" text-anchor="middle">${status}</text><rect x="60" y="225" width="150" height="180" rx="18" fill="#efe3d8"/><text x="135" y="330" fill="#8d594d" font-family="Arial" font-weight="700" font-size="54" text-anchor="middle">${xmlEscape(initials(member.name))}</text>${photo ? `<image href="${photoHref}" x="60" y="225" width="150" height="180" preserveAspectRatio="xMidYMid slice"/>` : ''}<text x="245" y="250" fill="#ffffff" fill-opacity=".65" font-family="Arial" font-size="14" letter-spacing="3">NAMA ANGGOTA</text><text x="245" y="293" fill="#ffffff" font-family="Arial" font-weight="700" font-size="34">${name}</text><text x="245" y="325" fill="#ffffff" fill-opacity=".75" font-family="Arial" font-size="16">${reg}</text><rect x="245" y="345" width="130" height="32" rx="16" fill="none" stroke="#f0a995" stroke-opacity=".75"/><text x="310" y="367" fill="#f0c4b4" font-family="Arial" font-size="13" text-anchor="middle">GOL. DARAH  ${blood}</text><rect x="690" y="245" width="126" height="126" rx="10" fill="#ffffff"/>${qrSrc ? `<image href="${qrHref}" x="697" y="252" width="112" height="112" preserveAspectRatio="xMidYMid meet"/>` : ''}<text x="690" y="425" fill="#ffffff" fill-opacity=".72" font-family="Arial" font-size="13" letter-spacing="3">RCS.CBS HOPE · 2026</text><text x="690" y="450" fill="#ffffff" fill-opacity=".7" font-family="Arial" font-size="12" letter-spacing="2">MEMBER ID</text></svg>`;
  const backSvg=`<svg xmlns="http://www.w3.org/2000/svg" width="860" height="540" viewBox="0 0 860 540"><defs><linearGradient id="back" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#f7f8f6"/><stop offset="1" stop-color="#ead9cc"/></linearGradient></defs><rect width="860" height="540" rx="38" fill="url(#back)" stroke="#ead8cb" stroke-width="3"/><image href="${compassHref}" x="565" y="72" width="245" height="245" preserveAspectRatio="xMidYMid slice" opacity=".94"/><text x="68" y="130" fill="#b83c35" font-family="Arial" font-weight="700" font-size="32">Bangga menjadi bagian dari</text><text x="68" y="172" fill="#b83c35" font-family="Arial" font-weight="700" font-size="32">keluarga Reichas.</text><text x="68" y="235" fill="#667478" font-family="Arial" font-size="18">Saling mengenal, saling menjaga.</text><text x="68" y="270" fill="#667478" font-family="Arial" font-size="17">Kartu ini adalah identitas anggota untuk kegiatan</text><text x="68" y="300" fill="#667478" font-family="Arial" font-size="17">Organisasi Reichas Chelebes.</text><text x="68" y="430" fill="#899695" font-family="Arial" font-size="15">${parent} · GOL. DARAH ${blood}</text><text x="68" y="462" fill="#899695" font-family="Arial" font-size="15">${reg}</text></svg>`;
