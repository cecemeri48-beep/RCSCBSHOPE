...
function openCardPreview(member) {
  ...
  body.innerHTML=`<div class="id-card-preview"><div class="id-card-face"><div class="membership-watermark">MEMBERS ONLY</div><div class="id-card-top"><img class="id-card-logo" src="${safe(logo)}" alt="Logo RCS.CBS HOPE" /><span class="id-card-badge">${safe(member.status || 'ANGGOTA')}</span></div><div class="id-card-main">${cardPhotoMarkup(member)}<div><span class="id-card-label">Nama anggota</span><div class="id-card-name">${safe(member.name).toUpperCase()}</div><div class="id-card-reg">${safe(member.registration_number || 'Nomor belum diterbitkan')}</div><div class="id-card-blood">GOL. DARAH <b>${safe(member.blood_type)}</b></div><div class="id-card-tagline">PENCINTA ALAM · COMMUNITY MEMBER</div></div></div><div class="id-card-qr"><canvas id="modalQr"></canvas></div><div class="id-card-bottom"><span>RCS.CBS HOPE</span><span>BERLAKU SEUMUR HIDUP</span><span>SELAMA KARTU AKTIF</span><span>MEMBER ID</span></div></div><div class="id-card-back"><img class="back-compass-img" src="${safe(compass)}" alt="Kompas mata angin" /><div class="back-copy"><span class="back-kicker">REICHAS CHELEBES</span><strong>Bangga menjadi bagian Keluarga Reichas Chelebes.</strong><p>Saling mengenal, saling menjaga. Kartu ini adalah identitas anggota untuk kegiatan organisasi pencinta alam.</p></div><div class="back-detail"><div class="back-detail-block"><span>PEMILIK LEGACY</span><strong>${safe(member.parent_name).toUpperCase()}</strong></div><div class="back-detail-block"><span>NAMA ANGKATAN</span><strong>${safe(member.cohort_name || '-').toUpperCase()}</strong></div></div></div></div><div class="modal-actions"><button class="btn btn-dark" id="modalPrintCard" type="button">Download Kartu PDF <b>↓</b></button><button class="btn btn-dark" id="modalDownloadPng" type="button">Simpan Kartu PNG <b>↓</b></button></div>`;
  renderQRCode($('#modalQr'), verifyUrl(member.public_token || member.id)); $('#modalPrintCard')?.addEventListener('click',()=>openPrintCard(member)); $('#modalDownloadPng')?.addEventListener('click',()=>openPrintCard(member,'png')); modal.classList.remove('hidden'); document.body.classList.add('modal-open');
}
...
async function openPrintCard(member, mode='pdf') {
  ...
  if (mode==='png') {
    ...
    try { const [frontImage,backImage]=await Promise.all([loadSvgImage(frontSvg),loadSvgImage(backSvg)]); const scale=2; const canvas=document.createElement('canvas'); canvas.width=860*scale; canvas.height=1120*scale; const ctx=canvas.getContext('2d'); ctx.fillStyle='#ffffff'; ctx.fillRect(0,0,canvas.width,canvas.height); ctx.drawImage(frontImage,0,0,860*scale,540*scale); ctx.drawImage(backImage,0,580*scale,860*scale,540*scale); canvas.toBlob(blob=>{if(!blob){toast('Kartu PNG gagal dibuat');w.close();return;}const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download=`kartu-${String(member.name||'anggota').toLowerCase().replace(/[^a-z0-9]+/g,'-')}.png`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>{URL.revokeObjectURL(url);w.close();},500);},'image/png'); } catch (_) { w.close(); toast('Kartu PNG gagal dibuat. Coba PDF.'); }
    return;
  }
