...
function openPrintCard(member) {
  ...
  const w = window.open('', '_blank', 'noopener,noreferrer,width=850,height=620');
  if (!w) { toast('Izinkan pop-up untuk mencetak kartu.'); return; }
  w.document.write(`<!doctype html><html><head><title>Kartu ${safe(member.name)}</title><style>@page{size:auto;margin:12mm}body{margin:0;background:#fff;font-family:Arial,sans-serif;color:#17212b}.card{width:86mm;height:54mm;background:linear-gradient(135deg,#19252f,#45616b);color:#fff;border-radius:4mm;padding:6mm;box-sizing:border-box;position:relative;overflow:hidden}.top{font-size:8pt;letter-spacing:1.2pt;display:flex;justify-content:space-between}.tag{font-size:6pt;border:1px solid rgba(255,255,255,.5);padding:2mm;border-radius:1mm}.body{position:absolute;left:6mm;bottom:15mm;display:flex;gap:4mm;align-items:center}.print-photo{width:17mm;height:20mm;border-radius:2mm;object-fit:cover;background:#efe4da}.print-placeholder{display:grid;place-items:center;color:#8a4d42;font-weight:bold;font-size:14pt}.label{font-size:6pt;color:rgba(255,255,255,.62);text-transform:uppercase;letter-spacing:1pt}.name{font-size:14pt;font-weight:bold;margin:1mm 0}.id{font-size:6.5pt;color:rgba(255,255,255,.68)}.bottom{position:absolute;left:6mm;right:6mm;bottom:5mm;display:flex;justify-content:space-between;font-size:6pt;letter-spacing:1pt;color:rgba(255,255,255,.7)}.qr{width:16mm;height:16mm;background:white;position:absolute;right:6mm;bottom:13mm;padding:1mm;border-radius:1mm}.qr img{width:100%;height:100%}.caption{font:10pt Arial;color:#69747a;margin-top:7mm}</style></head><body><div class="card"><div class="top"><span>RCS.CBS HOPE</span><span class="tag">MEMBER</span></div><div class="body">${photo}<div><div class="label">Nama anggota</div><div class="name">${safe(member.name).toUpperCase()}</div><div class="id">${safe(member.registration_number)}</div><div class="id">${safe(member.parent_name)} · ${safe(member.blood_type)}</div></div></div><div class="qr"><img id="qr" alt="QR" /></div><div class="bottom"><span>ANGGOTA AKTIF · 2026</span><span>RCS.CBS</span></div></div><div class="caption">Cetak kartu ini pada ukuran seperti KTP. Pilih “Save as PDF” bila ingin mengunduh.</div><script src="https://cdn.jsdelivr.net/npm/qrcode@1.5.4/build/qrcode.min.js"><\/script><script>QRCode.toDataURL(${JSON.stringify(qr)},{width:160,margin:1},function(e,u){document.getElementById('qr').src=u;setTimeout(function(){window.print()},350)})<\/script></body></html>`);
  w.document.close();
}
...
$('#registrationForm').addEventListener('submit', async (event) => {
  ...
  try {
    ...
    setTimeout(()=>{ $('#verifikasi').scrollIntoView({behavior:'smooth'}); renderResult(member); }, 450);
...
async function loadMembers() {
  ...
  $$('.view-member',tbody).forEach(el=>el.addEventListener('click',()=>{const m=items.find(x=>x.id===el.dataset.id);renderResult(m);$('#verifikasi').scrollIntoView({behavior:'smooth'});}));
}
...
function routeFromHash(){const hash=window.location.hash;if(hash.startsWith('#verify=')){const token=decodeURIComponent(hash.slice(8));$('#verifyInput').value=token;$('#verifikasi').scrollIntoView({behavior:'smooth'});findMember(token).then(renderResult).catch(()=>renderResult(null));}}
