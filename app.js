...
async function loadMembers() {
  ...
  const tbody=$('#membersTable'); if(!items.length){tbody.innerHTML='<tr><td colspan="7" class="table-empty">Belum ada data.</td></tr>';return;} tbody.innerHTML=items.map(m=>`<tr><td><div class="registration-edit"><input class="registration-input" data-id="${safe(m.id)}" value="${safe(m.registration_number||'')}" placeholder="Nomor registrasi" /><button class="btn btn-outline small save-registration" data-id="${safe(m.id)}" type="button">Simpan</button></div></td><td><b>${safe(m.name)}</b></td><td>${safe(m.parent_name)}</td><td>${safe(m.parent_phone||'—')}</td><td title="${safe(m.parent_address||'—')}">${safe(m.parent_address||'—')}</td><td><select class="status-select" data-id="${safe(m.id)}"><option ${m.status==='Menunggu Verifikasi'?'selected':''}>Menunggu Verifikasi</option><option ${m.status==='Perlu Perbaikan'?'selected':''}>Perlu Perbaikan</option><option ${m.status==='Aktif'?'selected':''}>Aktif</option><option ${m.status==='Nonaktif'?'selected':''}>Nonaktif</option></select></td><td><div class="table-actions"><button class="btn btn-outline small view-member" data-id="${safe(m.id)}" type="button">Lihat</button><button class="btn btn-danger small delete-member" data-id="${safe(m.id)}" type="button">Hapus</button></div></td></tr>`).join('');
