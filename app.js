...
function renderAdminRows(){
  const items=adminMembers; const duplicateCounts={}; items.forEach(m=>{const key=[m.name,m.parent_name].map(v=>String(v||'').trim().toUpperCase()).join('::'); if(key!=='::') duplicateCounts[key]=(duplicateCounts[key]||0)+1;}); const search=($('#adminSearch')?.value||'').trim().toLowerCase(); const status=($('#adminStatusFilter')?.value||'');
