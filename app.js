...
function generateMemberId(sequence){const clean=String(sequence||'').trim();if(!/^\d{1,4}$/.test(clean)){toast('Masukkan nomor urut admin terlebih dahulu, contoh 001');return null;}const ordinal=Number(clean);if(ordinal<1||ordinal>3999){toast('Nomor urut harus antara 001 sampai 3999');return null;}const bytes=new Uint8Array(5);if(window.crypto?.getRandomValues)window.crypto.getRandomValues(bytes);else for(let i=0;i<bytes.length;i++)bytes[i]=Math.floor(Math.random()*256);const secret=[...bytes].map(x=>x.toString(36).toUpperCase().padStart(2,'0')).join('').slice(0,6);const number=1000+((((bytes[0]<<8)|bytes[1])%9000));return `RCS.CBS · ${arabicIndic(number)} · ${romanNumber(ordinal)} · ${secret}`;}
