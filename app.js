...
$('#registrationForm').addEventListener('submit', async (event) => {
  ...
  try {
    ...
    const payload = { name:data.get('name').trim(), parent_name:data.get('parent_name').trim(), cohort_name:data.get('cohort_name').trim(), cohort_year:null, blood_type:data.get('blood_type'), parent_phone:data.get('parent_phone').trim(), photo_path:photo.path, photo_url:photo.url, consent:Boolean(data.get('consent')), status:'Menunggu Verifikasi' };
    let member;
