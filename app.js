...
const COMPETENCY_TRACKS = Object.freeze([
  { code:'rimba', label:'JEJAK RIMBA' },
  { code:'vertikal', label:'JEJAK VERTIKAL' },
  { code:'speleo', label:'JEJAK SPELEO' },
  { code:'maritim', label:'JEJAK MARITIM' },
  { code:'kultura', label:'JEJAK KULTURA' },
  { code:'mastermind', label:'JEJAK MASTERMIND' }
]);
  ...
  if (isConfigured()) {
    ...
    if (error) {
      if (error.code === 'PGRST202' || /verify_member/i.test(error.message || '')) {
        throw new Error('Layanan verifikasi belum diperbarui. Jalankan supabase/hotfix-v97-competencies.sql di Supabase SQL Editor.');
      }
      throw error;
    }
